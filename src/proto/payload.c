...
static atomic_t d_cache;      /* latest D payload (packed u32) */
...
{
    atomic_set(&d_cache, (atomic_val_t)pack4(v));  /* publish value first */
    atomic_or(&ready_flags, FLAG_C_READY);         /* then mark as ready */
}
...
void snapshot(uint8_t cmd, uint8_t out[4])
{
    ...
    {
        uint32_t u = (uint32_t)atomic_get(&d_cache);
        unpack4(u, out);
    } 
    else 
    {
...
void clear_ready(uint8_t cmd)
{
    ...
    {
        atomic_and(&ready_flags, ~FLAG_D_READY);
    }
    /* Unknown cmd: ignore */
}
